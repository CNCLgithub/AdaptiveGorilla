################################################################################
# Script to run models on Sensitivity - Study 1
#
...
GORILLA_COLORS = [Light, Dark]
NC = length(GORILLA_COLORS)
...
function main()
    ...
    linds = LinearIndices((CHAINS, NC, 2))
    # Go through each of the conditions
    for (i, color) = enumerate(GORILLA_COLORS),
        (j, param_val ) = enumerate(MODEL_PARAM_LOW_HIGH)
    ...
    out_dir = "/spaths/experiments/sensitivity-1/$(MODEL_PARAM_KEY)/$(ANALYSIS)"
